...
def execute_plan(tasks: list[dict], org_id: int = None,
    ...
    for i, task in independent:
        ...
        task_ids[i] = task_id
        dispatch_task(task, task_id, org_id=org_id, project_goal=project_goal)
    ...
    for i, task in dependent:
        ...
        dispatch_task(task, task_id, context, org_id=org_id,
                     project_goal=project_goal)
...
def recent_tasks():
    ...
    try:
        ...
        conn.close()
        tasks_list = [
            {"agent": r[0], "task": r[1][:100], "status": r[2],
...
def _require_auth():
    """Check Bearer token. Returns error response or None if OK."""
    if not BOSS_API_KEY:
        return jsonify({"error": "BOSS_API_KEY not configured"}), 500
    auth = request.headers.get("Authorization", "")
    if not hmac.compare_digest(auth, f"Bearer {BOSS_API_KEY}"):
...
def config_snapshot():
    ...
    try:
        ...
        conn = psycopg2.connect(POSTGRES_URL)
        cur = conn.cursor()
        # Gather current budgets + autonomy + registry into one JSON blob
